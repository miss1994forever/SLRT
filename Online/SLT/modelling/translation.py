...
class TranslationNetwork(torch.nn.Module):
    ...
    def generate(self, 
        ...
        # wait_k = kwargs.pop('wait_k', 0)
        # move_step = kwargs.pop('move_step', 1)
        # if wait_k > 0:
        #     assert batch_size == 1
        ...
        #     start = min(wait_k, T)
        #     final_res = []
        #     temp_mask = torch.zeros_like(attention_mask).long()
        #     for end in range(start, T+move_step, move_step):
        #         temp_mask[:, :end] = 1
        #         output_dict = self.model.generate(
        #             inputs_embeds=inputs_embeds, attention_mask=attention_mask*temp_mask, #same with forward 
