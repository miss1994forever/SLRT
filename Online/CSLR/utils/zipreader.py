...
class ZipReader(object):
    ...
    def read(path):
        ...
        if ZipReader.is_directory_root(zip_path):
            file_path = os.path.join(zip_path, path_img)
            try:
                ...
                if len(parts) == 3 and parts[0] == 'images' and parts[1] in {'train', 'dev', 'test'}:
                    with open(os.path.join(zip_path, 'images', parts[2]), 'rb') as handle:
                        return handle.read()
                raise
        ...
        try:
            ...
            parts = path_img.split('/', 2)
            if len(parts) == 3 and parts[0] == 'images' and parts[1] in {'train', 'dev', 'test'}:
                fallback_path = 'images/' + parts[2]
                if hasattr(zfile, 'extractfile'):
                    handle = zfile.extractfile(fallback_path)
                    if handle is None:
                        raise
                    data = handle.read()
                else:
                    data = zfile.read(fallback_path)
            else:
                raise
        return data