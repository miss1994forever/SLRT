...
if not hasattr(distutils, 'version'):
    class _DistutilsVersionModule:
        LooseVersion = _LooseVersion
    distutils.version = _DistutilsVersionModule()
...
def _normalize_fusion_weights(rgb_weight, keypoint_weight, fuse_weight):
...
def select_gloss_logits(forward_output, pred_src, rgb_weight=0.10, keypoint_weight=0.70, fuse_weight=0.20):
    if pred_src == 'ensemble':
        return forward_output['ensemble_last_gloss_logits']
    if pred_src == 'rgb':
        return forward_output['rgb_gloss_logits']
    if pred_src == 'keypoint':
        return forward_output['keypoint_gloss_logits']
    if pred_src == 'fuse':
        return forward_output['fuse_gloss_logits']
    if pred_src == 'weighted':
        rgb_w, keypoint_w, fuse_w = _normalize_fusion_weights(rgb_weight, keypoint_weight, fuse_weight)
        rgb_prob = forward_output['rgb_gloss_logits'].softmax(dim=-1)
        keypoint_prob = forward_output['keypoint_gloss_logits'].softmax(dim=-1)
        fuse_prob = forward_output['fuse_gloss_logits'].softmax(dim=-1)
...
def sliding_windows(video, keypoint, win_size=16, stride=1, save_fea=False, adaptive_cfg=None, return_starts=False):
    ...
    if return_starts:
        return video_s, keypoint_s, starts, window_metadata
...
def evaluation_slide(model, cslr_dataloader, cfg, 
        ...
        rgb_weight=0.10, keypoint_weight=0.70, fuse_weight=0.20,
        max_samples=None):
    logger = get_logger()
    ...
    span_voting_min_weight = float(span_voting_cfg.get('min_weight', 0.05))
    ...
    if use_span_voting:
        decode_method_lst.insert(-1, 'span_weighted_{}'.format(span_voting_size))
    ...
    with torch.no_grad():
        ...
        for thr in thr_lst:
            ...
            logits_dict = {}
            norm_rgb_w, norm_keypoint_w, norm_fuse_w = _normalize_fusion_weights(
            ...
            for step, batch in enumerate(cslr_dataloader):
                if max_samples is not None and step >= max_samples:
                    logger.info(f'Stop early after max_samples={max_samples}')
                    break
                #forward -- loss
                name = batch['names'][0]
                ...
                video_s, keypoint_s, window_starts, window_metadata = sliding_windows(
                    batch['sgn_videos'][0],
                ...
                for v_s, k_s in zip(video_s_lst, keypoint_s_lst):
                    ...
                    split_starts = window_starts[window_offset:window_offset + v_s.size(0)]
                    split_centers = torch.tensor(
                        [start + (win_size - 1) / 2.0 for start in split_starts],
                        dtype=gls_logits.dtype,
                        device=gls_logits.device,
                    )
                    window_offset += v_s.size(0)
                    ...
                    # if torch.sum(max_prob > thr).item() > 0:
                        ...
                        final_window_centers.append(split_centers[max_prob > thr])
                    # else:
                ...
                if final_decode_op:
                    ...
                    final_window_centers = torch.cat(final_window_centers, dim=0)
                else:
                    final_decode_op = torch.cat(all_decode_op, dim=0)
                    final_gls_logits = torch.cat(all_gls_logits, dim=0)
                    final_window_centers = torch.cat(all_window_centers, dim=0)
                ...
                results[name]['adaptive_stride_metadata'] = window_metadata
                ...
                if use_span_voting:
                    span_index = span_weighted_predictions(
                        final_gls_logits,
                        final_window_centers.tolist(),
                        span=span_voting_size,
                        min_weight=span_voting_min_weight,
                    ).detach().cpu().numpy()
                    filtered_span_index = []
                    for token_id in span_index:
                        if not filtered_span_index or token_id != filtered_span_index[-1]:
                            filtered_span_index.append(token_id)
                    filtered_span_index = [token_id for token_id in filtered_span_index if token_id != blank_id]
                    span_gls_hyp = index2token(filtered_span_index, vocab, dataset_name)
                    results[name]['span_weighted_{}_gls_hyp'.format(span_voting_size)] = ' '.join(span_gls_hyp)
                ...
                if 'window_greedy_7' in decode_method_lst:
                    filter_logits = {}
                    base_gls_logits = final_gls_logits
                    base_rgb_feas = feas['rgb_blk5'][name] if (use_bag_fc or model_ex) else None
                    base_kp_feas = feas['keypoint_blk5'][name] if (use_bag_fc or model_ex) else None
                    for decode_win_size in [3,5,7,9,11,13]:
                        ...
                        # pad logits
                        padded_gls_logits = pad_tensor(base_gls_logits, decode_win_size//2, decode_win_size//2)
                        
                        if use_bag_fc or model_ex:
                            rgb_feas = pad_tensor(base_rgb_feas, decode_win_size//2, decode_win_size//2).to(model.device)
                            kp_feas = pad_tensor(base_kp_feas, decode_win_size//2, decode_win_size//2).to(model.device)
                        ...
                        for st in range(index.shape[0] - decode_win_size + 1):
                            ...
                            if use_bag_fc or model_ex:
                                ...
                                logits = padded_gls_logits[st:st+decode_win_size]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--adaptive_min_stride', default=None, type=int)
    parser.add_argument('--adaptive_max_stride', default=None, type=int)
    parser.add_argument('--adaptive_ema_decay', default=None, type=float)
    ...
    parser.add_argument('--split_size', default=None, type=int, help='Override inference clip micro-batch size.')
    parser.add_argument('--split', default='test', type=str)
    parser.add_argument('--save_fea', default=0, choices=[0,1], type=int)
    parser.add_argument('--pred_src', default='ensemble', choices=['ensemble', 'rgb', 'keypoint', 'fuse', 'weighted'], type=str)
    parser.add_argument('--rgb_weight', default=0.10, type=float)
    ...
    if args.split_size is not None:
        ...
        cfg['data']['split_size'] = args.split_size
    adaptive_cfg = dict(cfg['data'].get('adaptive_stride', {}))
    cli_adaptive_values = {
        'enabled': None if args.adaptive_stride is None else bool(args.adaptive_stride),
        'min_stride': args.adaptive_min_stride,
        'max_stride': args.adaptive_max_stride,
        'ema_decay': args.adaptive_ema_decay,
        'keypoint_confidence_threshold': args.adaptive_confidence_threshold,
    ...
    cli_span_values = {
        'enabled': None if args.span_weighted_voting is None else bool(args.span_weighted_voting),
        'vote_span_frames': args.vote_span_frames,
        'min_weight': args.span_min_weight,
    }
    span_cfg.update({key: value for key, value in cli_span_values.items() if value is not None})
    postprocess_cfg['span_weighted_voting'] = span_cfg
    cfg['postprocess'] = postprocess_cfg
    ...
    logger.info(
        'Prediction slide source: pred_src=%s, rgb_weight=%.4f, keypoint_weight=%.4f, fuse_weight=%.4f',
        args.pred_src, args.rgb_weight, args.keypoint_weight, args.fuse_weight
    )
    ...
    for split in [args.split]:
        ...
        if cfg['task'] == 'ISLR':
            _, _ = evaluation_slide(model=model, cslr_dataloader=dataloader, cfg=cfg, 
                    ...
                    fuse_weight=args.fuse_weight, max_samples=args.max_samples)
