...
> 口径状态：**正式最终结果**。统一摘要、指标定义和机器数据位置见 `docs/RESULTS.md`。
...
这里的“实际时间”统一定义为单次完整命令的 wall-clock `real` 时间，包含初始化、数据加载、模型前向、解码和结果保存，不是纯 GPU 时间或在线 P95 延迟。WER/clips 可从 PKL 复核；wall time 来自当次 `/usr/bin/time` 记录。
