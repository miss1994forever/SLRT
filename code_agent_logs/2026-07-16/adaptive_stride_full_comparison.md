...
> 口径状态：**历史工程对照，不是冻结最终结果**。本文件使用 span-13；之后只用 dev 选择出的正式配置为 span-15，最终 test WER 23.0571%、clips -32.02%、完整命令 wall time -26.25%。对外统一引用 `docs/RESULTS.md`，不得把本文件的 span-13 WER/runtime 与 span-15 最终结果拼接。
