...
执行前审计（2026-07-21）：已有产物没有保存同一次前向的三路完整逐样本记录，因此无法直接计算联合分歧和按可靠性选流效果。R1 将对 isolated dev 做一次无训练前向，并只保存标量诊断记录；不读取 test。预注册输出包括正确性 AUROC、10-bin ECE、置信度与正确性相关、25/50/75/100% coverage 准确率、三流一致/分歧率、oracle 任一流正确率、最大置信度/最小熵选流准确率、Jensen-Shannon 分歧，以及关键点可见率、平均置信度和运动量的正确率分箱。看到结果后不增加或删除指标。

完成状态（2026-07-26）：完整 13,077 样本 dev 前向已完成，未读取 test。三流置信度的正确性 AUROC 为 0.754--0.791，说明流内置信度可用于拒识；但最大原始置信度选流为 63.86%，低于 Keypoint-only 的 66.28%。因此进入 R2 时禁止使用未校准的直接选流，改为 signness 分组校准和 Keypoint-default 保守覆盖。完整结果见 `../2026-07-26/reliability_r1_results.md`。

### R2：启发式可靠性融合

在 dev 上预注册少量融合候选，与 equal ensemble 和固定权重对照。指标包括 WER、DEL/INS/SUB、重复率、blank 率和额外解码耗时。

R1 后约束：候选至少包含 Keypoint-only；可靠性按 blank/nonblank 分开校准；动态规则默认保持 Keypoint，仅在流间分歧且其他流有足够校准优势时覆盖。校准优先使用 train 统计；如需 dev 校准，必须先固定 calibration/evaluation 划分，避免同集拟合与报告。
